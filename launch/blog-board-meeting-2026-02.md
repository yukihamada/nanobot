# ブログ記事: 経営会議レポート 2026年2月

## タイトル案

1. **「月800ドルのLLMコストを52ドルに。個人開発AIサービスの生存戦略」**
2. **「210人のユーザーと3万リクエスト/日 — chatweb.aiの"赤裸々"経営会議を公開します」**
3. **「日本語特化AIモデルで運営コスト1/15に。技術選定の裏側を全部見せます」**

---

## 本文（タイトル案2を採用）

# 210人のユーザーと3万リクエスト/日 — chatweb.aiの"赤裸々"経営会議を公開します
...
ここで登場するのがNVIDIAの**Nemotron Nano 9B v2 Japanese**。DeepInfra上で動くこのモデルの価格を見たとき、思わず二度見しました。
...
会議中に出たもう一つの話題。開発者向けブランドとして立ち上げた**teai.io**のトップページに、なぜか**chatweb.ai**のコンテンツが表示されるというバグがありました。

原因はシンプルで、Rustバイナリに`include_str!()`でHTMLを埋め込んでいるのですが、ホスト名の判定分岐が一部抜けていたこと。修正自体は数行で済みましたが、「開発者向けサービスのトップページが別サービスのページ」という状況は、なかなかの恥ずかしさでした。教訓: **シングルバイナリアーキテクチャの罠は、全部が一つにまとまっているがゆえに起きる**。
...
1. **今すぐ**: teai.ioトップページの修正デプロイ
2. **今週**: Nemotron日本語モデルの本番投入（LLMコスト$800→$52へ）
3. **来週**: Freeプランのクレジット増量でユーザー体験改善
4. **今月**: Product Hunt & Hacker Newsでのローンチ
...
*chatweb.ai は日本発の音声ファーストAIアシスタントです。[chatweb.ai](https://chatweb.ai) で無料で試せます。開発者の方は [teai.io](https://teai.io) もご覧ください。*
