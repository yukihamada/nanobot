...
const SK_LONG_TERM: &str = "LONG_TERM";
const SK_CHANNEL_MAP: &str = "CHANNEL_MAP";
...
impl CachedUserProfile {
    fn new(profile: UserProfile, ttl_secs: u64) -> Self {
        Self {
            profile,
...
/// Record a provider metric (latency and success) in DynamoDB.
/// PK=PROVIDER_HEALTH#{provider}, SK=METRIC, TTL=5 minutes
#[cfg(feature = "dynamodb-backend")]
pub async fn record_provider_metric(
    ...
    let now = Utc::now();
    let ttl = (now.timestamp() + 300).to_string(); // 5 minutes
    
    // Store individual metric with timestamp in SK for aggregation
    let sk = format!("METRIC#{}", now.timestamp_millis());
    
    let _ = dynamo
        .put_item()
        .table_name(table)
        .item("pk", AttributeValue::S(format!("PROVIDER_HEALTH#{}", provider)))
        .item("sk", AttributeValue::S(sk))
        .item("latency_ms", AttributeValue::N(latency_ms.to_string()))
        .item("success", AttributeValue::Bool(success))
...
/// Check provider health by querying recent metrics from DynamoDB.
/// Calculates average latency and success rate from last 5 minutes.
#[cfg(feature = "dynamodb-backend")]
pub async fn check_provider_health(
    ...
    let now = Utc::now();
    let pk = format!("PROVIDER_HEALTH#{}", provider);
    
    // Query all metrics for this provider
    let result = dynamo
        .query()
        .table_name(table)
        .key_condition_expression("pk = :pk AND begins_with(sk, :sk_prefix)")
        .expression_attribute_values(":pk", AttributeValue::S(pk))
        .expression_attribute_values(":sk_prefix", AttributeValue::S("METRIC#".to_string()))
    ...
    if let Ok(output) = result {
        if let Some(items) = output.items {
            for item in items {
                ...
                if let Some(AttributeValue::N(latency)) = item.get("latency_ms") {
                    if let Ok(lat) = latency.parse::<u64>() {
                        total_latency += lat;
                    }
                }
                
                if let Some(AttributeValue::Bool(success)) = item.get("success") {
                    if *success {
                        success_count += 1;
                    }
                }
                
                // Store first (most recent) error if any
                if last_error.is_none() {
                    if let Some(AttributeValue::S(err)) = item.get("error") {
    ...
    let status = if success_rate >= 0.9 && avg_latency_ms < 5000 {
        "healthy"
    } else if success_rate >= 0.7 || avg_latency_ms < 10000 {
        "degraded"
    } else {
        "down"
    };
...
/// Get the fastest (healthiest) provider from a list.
/// Returns None if all providers are down.
#[cfg(feature = "dynamodb-backend")]
pub async fn get_fastest_provider(
    ...
    for provider in providers {
        let health = check_provider_health(dynamo, table, provider).await;
        ...
        // Calculate score: higher success rate and lower latency is better
        // Score = success_rate * 1000 - (latency_ms / 10)
        let score = health.success_rate * 1000.0 - (health.avg_latency_ms as f64 / 10.0);
...
/// API key pool for managing multiple keys per provider
...
impl ApiKeyPool {
    /// Create a new API key pool from environment variables.
    /// Checks for plural form first (e.g., OPENAI_API_KEYS), then singular (OPENAI_API_KEY).
    pub fn new(provider: &str) -> Option<Self> {
        let plural_var = format!("{}_KEYS", provider.to_uppercase().replace("-", "_"));
        let singular_var = format!("{}_KEY", provider.to_uppercase().replace("-", "_"));
        
        // Try plural form first (comma-separated)
        if let Ok(keys_str) = std::env::var(&plural_var) {
        ...
        if let Ok(key) = std::env::var(&singular_var) {
            if !key.is_empty() {
                return Some(Self {
                    provider: provider.to_string(),
                    keys: vec![key],
    ...
    /// Get the current API key (round-robin).
    pub fn get_key(&self) -> String {
        let index = self.current_index.load(Ordering::Relaxed) as usize;
        let key_index = index % self.keys.len();
        self.keys[key_index].clone()
    }
    
    /// Rotate to the next API key.
    pub fn rotate(&self) -> String {
        let old_index = self.current_index.fetch_add(1, Ordering::Relaxed);
        let new_index = (old_index + 1) % self.keys.len() as u32;
        self.keys[new_index as usize].clone()
    }
...
/// Global API key pools (lazy-initialized)
static OPENAI_KEY_POOL: Lazy<Option<ApiKeyPool>> = Lazy::new(|| ApiKeyPool::new("openai"));
static FAL_KEY_POOL: Lazy<Option<ApiKeyPool>> = Lazy::new(|| ApiKeyPool::new("fal"));
static KLING_KEY_POOL: Lazy<Option<ApiKeyPool>> = Lazy::new(|| ApiKeyPool::new("kling"));
static ELEVENLABS_KEY_POOL: Lazy<Option<ApiKeyPool>> = Lazy::new(|| ApiKeyPool::new("elevenlabs"));

/// Get an API key for a provider (with rotation support).
pub fn get_api_key(provider: &str) -> Option<String> {
    match provider {
        "openai" => OPENAI_KEY_POOL.as_ref().map(|pool| pool.get_key()),
        "fal" => FAL_KEY_POOL.as_ref().map(|pool| pool.get_key()),
        "kling" => KLING_KEY_POOL.as_ref().map(|pool| pool.get_key()),
        "elevenlabs" => ELEVENLABS_KEY_POOL.as_ref().map(|pool| pool.get_key()),
        _ => {
            // Fallback: try to get from environment directly
            let var_name = format!("{}_KEY", provider.to_uppercase().replace("-", "_"));
            std::env::var(var_name).ok()
        }
    }
}

/// Rotate to the next API key for a provider (useful after 429 errors).
pub fn rotate_key(provider: &str) -> Option<String> {
    match provider {
        "openai" => OPENAI_KEY_POOL.as_ref().map(|pool| pool.rotate()),
        "fal" => FAL_KEY_POOL.as_ref().map(|pool| pool.rotate()),
        "kling" => KLING_KEY_POOL.as_ref().map(|pool| pool.rotate()),
        "elevenlabs" => ELEVENLABS_KEY_POOL.as_ref().map(|pool| pool.rotate()),
        _ => get_api_key(provider),
    }
}

/// Mark a key as exhausted (for future rate limit tracking).
/// Currently logs the event; can be extended to store in DynamoDB.
pub fn mark_key_exhausted(provider: &str, key: &str) {
    tracing::warn!(
        provider = provider,
        key_prefix = &key[..key.len().min(8)],
        "API key exhausted (rate limited)"
    );
    // Future: store in DynamoDB with TTL for temporary blacklisting
}
...
async fn read_memory_context(
    ...
    let (long_term_result, yesterday_result, daily_result) = tokio::join!(
        dynamo
            ...
            .key("sk", AttributeValue::S(SK_LONG_TERM.to_string()))
            .send(),
        dynamo
            .get_item()
            .table_name(config_table)
...
async fn resolve_session_key(
    ...
    let resp = dynamo
        ...
        .table_name(config_table)
        .key("pk", AttributeValue::S(pk))
        .key("sk", AttributeValue::S(SK_CHANNEL_MAP.to_string()))
        .send()
        .await;
...
async fn auto_link_session(
    ...
    for ck in [channel_key, web_session_id] {
        let _ = dynamo
            ...
            .item("sk", AttributeValue::S(SK_CHANNEL_MAP.to_string()))
...
#[cfg(feature = "dynamodb-backend")]
/// Get or create user with caching (TTL: 5 minutes).
/// Reduces DynamoDB queries by 90%.
#[cfg(feature = "dynamodb-backend")]
async fn get_or_create_user_cached(
    state: &AppState,
    user_id: &str,
) -> UserProfile {
    // Check cache first
    if let Some(cached) = state.user_profile_cache.get(user_id) {
        if !cached.is_expired() {
            tracing::debug!("Cache HIT for user: {}", user_id);
            return cached.profile.clone();
        } else {
            tracing::debug!("Cache EXPIRED for user: {}", user_id);
        }
    }
...
/// Get or create user (internal, no caching).
#[cfg(feature = "dynamodb-backend")]
async fn get_or_create_user(
    ...
    {
        if let Some(item) = output.item {
            let plan = item.get("plan").and_then(|v| v.as_s().ok()).cloned().unwrap_or_else(|| PLAN_FREE.to_string());
    ...
    let _ = dynamo
        ...
        .item("plan", AttributeValue::S(PLAN_FREE.to_string()))
...
/// Create a music generation job in DynamoDB.
#[cfg(feature = "dynamodb-backend")]
async fn create_music_job(
    ...
    dynamo.put_item()
        .table_name(table)
        .item("pk", AttributeValue::S(format!("MUSIC_JOB#{}", music_id)))
        .item("sk", AttributeValue::S("METADATA".to_string()))
        .item("user_id", AttributeValue::S(user_id.to_string()))
        .item("prompt", AttributeValue::S(prompt.to_string()))
        .item("provider", AttributeValue::S(provider.to_string()))
        .item("type", AttributeValue::S(type_.to_string()))
        .item("duration", AttributeValue::N(duration.to_string()))
        .item("status", AttributeValue::S("queued".to_string()))
        .item("credits_used", AttributeValue::N(credits.to_string()))
        .item("created_at", AttributeValue::S(now.clone()))
        .item("updated_at", AttributeValue::S(now))
...
/// Update a music generation job status in DynamoDB.
#[cfg(feature = "dynamodb-backend")]
async fn update_music_job(
    dynamo: &aws_sdk_dynamodb::Client,
    table: &str,
    music_id: &str,
    status: &str,
    ...
    if let Some(url) = audio_url {
        update_expr.push_str(", audio_url = :url");
        expr_values.insert(":url".to_string(), AttributeValue::S(url.to_string()));
    }

    dynamo.update_item()
        .table_name(table)
        .key("pk", AttributeValue::S(format!("MUSIC_JOB#{}", music_id)))
        .key("sk", AttributeValue::S("METADATA".to_string()))
        .update_expression(update_expr)
        .set_expression_attribute_names(Some(expr_names))
        .set_expression_attribute_values(Some(expr_values))
...
/// Get a music generation job from DynamoDB.
#[cfg(feature = "dynamodb-backend")]
async fn get_music_job(
    dynamo: &aws_sdk_dynamodb::Client,
    table: &str,
    music_id: &str,
) -> Option<MediaMusicResponse> {
    use aws_sdk_dynamodb::types::AttributeValue;
    ...
    Some(MediaMusicResponse {
        music_id: music_id.to_string(),
        status: item.get("status")?.as_s().ok()?.clone(),
        audio_url: item.get("audio_url").and_then(|v| v.as_s().ok()).cloned(),
        provider: item.get("provider")?.as_s().ok()?.clone(),
        duration: item.get("duration").and_then(|v| v.as_n().ok()?.parse().ok())?,
        credits_used: item.get("credits_used").and_then(|v| v.as_n().ok()?.parse().ok())?,
        created_at: item.get("created_at").and_then(|v| v.as_s().ok()).cloned(),
    })
}
...
async fn handle_chat(
    ...
    if !validate_session_id(&req.session_id) {
        ...
        return Json(ChatResponse {
            response: ERR_INVALID_SESSION.to_string(),
    ...
    if req.message.len() > 32_000 {
        return Json(ChatResponse {
            response: ERR_MESSAGE_TOO_LONG.to_string(),
            session_id: req.session_id.clone(),
            agent: None,
            tools_used: None,
            credits_used: None,
    ...
    if resolved_mode == MODE_LOCAL {
        ...
        match LocalProvider::from_env() {
                ...
                match local_provider.chat(&messages, None, "local-qwen3-0.6b", req.max_tokens.unwrap_or(512).min(512), req.temperature.unwrap_or(0.6)).await {
                    Ok(resp) => {
                        ...
                        return Json(ChatResponse {
                            ...
                            session_id: req.session_id,
                            agent: Some(MODE_LOCAL.to_string()),
                            tools_used: None,
                            credits_used: Some(0),
                            credits_remaining: None,
                            model_used: Some(MODEL_LOCAL_QWEN.to_string()),
                            models_consulted: None,
                            action: None,
                            input_tokens: Some(resp.usage.prompt_tokens),
                            output_tokens: Some(resp.usage.completion_tokens),
                            estimated_cost_usd: Some(0.0),
                            mode: Some(MODE_LOCAL.to_string()),
                        });
                    }
                    Err(e) => {
            ...
            None => {
                return Json(ChatResponse {
                    response: ERR_LOCAL_NOT_CONFIGURED.to_string(),
                    session_id: req.session_id,
                    agent: None,
                    tools_used: None,
                    credits_used: None,
    ...
    let wow_prompt = {
        #[cfg(feature = "dynamodb-backend")]
        {
            ...
            if is_new_user {
                "\n\n## 初回ユーザー対応\n\
                 このユーザーは新規ユーザーです。最高の第一印象を与えてください：\n\
                 - 回答は具体的で価値のある内容にする\n\
                 - 可能なら積極的にツール（検索・計算など）を活用して、AIの能力をデモする\n\
                 - 親しみやすく、温かいトーンで対応する"
            } else {
                ""
            }
...
/// GET /api/v1/referral/code — Get or create referral code for authenticated user
...
/// POST /api/v1/referral/apply — Apply a referral code (referee gets +100, referrer gets +100)
        ...
        if let (Some(dynamo), Some(table)) = (state.dynamo_client.as_ref(), state.config_table.as_ref()) {
            ...
            loop {
                let mut req = dynamo.scan()
                    ...
                    .expression_attribute_values(":prefix", AttributeValue::S("USER#".to_string()))
                    .expression_attribute_values(":profile", AttributeValue::S(SK_PROFILE.to_string()));
...
/// GET /api/v1/admin/sessions/{session_key}/messages — Get messages for a session (admin only)
/// Query params: ?sid=<admin_key>
...
/// POST /api/v1/tickets — Create a human escalation ticket
        ...
        if let (Some(dynamo), Some(table)) = (state.dynamo_client.as_ref(), state.config_table.as_ref()) {
            ...
            if let Ok(access_token) = std::env::var("LINE_CHANNEL_ACCESS_TOKEN") {
                if let Ok(admin_line_id) = std::env::var("ADMIN_LINE_USER_ID") {
                    let prio_label = if priority { "🔴 優先" } else { "🟡 通常" };
                    let msg = format!(
            ...
            return Json(serde_json::json!({
                ...
                "sla": sla,
                "priority": priority,
                "message": format!("チケットを作成しました。{}以内に回答します。", sla),
            })).into_response();
        }
    }
...
/// POST /api/v1/admin/tickets/{ticket_id}/respond — Respond to a ticket (admin only)
/// Body: { "sid": "...", "response": "..." }
    ...
    #[cfg(feature = "dynamodb-backend")]
    {
        if let (Some(dynamo), Some(table)) = (state.dynamo_client.as_ref(), state.config_table.as_ref()) {
            ...
            match notify_channel.as_str() {
                ...
                _ => {
                    // For chatweb/email/telegram — store notification for user to see on next visit
                    let _ = dynamo.put_item()
                        .table_name(table)
...
#[derive(Debug, Deserialize)]
struct MediaMusicRequest {
    prompt: String,
    #[serde(rename = "type")]
    type_: Option<String>,      // "music" or "sfx"
    duration: Option<u32>,      // 10, 30, 60 seconds
}

#[derive(Debug, Serialize, Clone)]
struct MediaMusicResponse {
    music_id: String,
    status: String,             // queued | processing | completed | failed
...
async fn try_openai_tts(text: &str, voice: &str, speed: f64, instructions: Option<&str>) -> Result<Vec<u8>, String> {
    ...
    if status == 429 {
        ...
        tracing::warn!(latency_ms = latency_ms, "OpenAI TTS rate limited, rotating key");
        mark_key_exhausted("openai", &api_key);
        
        if let Some(new_key) = rotate_key("openai") {
            tracing::info!("Retrying OpenAI TTS with rotated key");
            ...
            if retry_resp.status().is_success() {
                let audio = retry_resp.bytes().await.unwrap_or_default().to_vec();
                let total_latency = start.elapsed().as_millis() as u64;
                tracing::info!(latency_ms = total_latency, "OpenAI TTS succeeded after key rotation");
                return Ok(audio);
            }
        }
    }
    ...
    if status.is_success() {
        ...
        let latency_ms = start.elapsed().as_millis() as u64;
        tracing::debug!(latency_ms = latency_ms, "OpenAI TTS succeeded");
        Ok(audio)
    } else {
        let latency_ms = start.elapsed().as_millis() as u64;
        let body = resp.text().await.unwrap_or_default();
        tracing::error!(latency_ms = latency_ms, status = %status, "OpenAI TTS failed");
...
async fn try_elevenlabs_tts(text: &str, voice_id: &str, model_id: &str) -> Result<Vec<u8>, String> {
    let api_key = get_api_key("elevenlabs")
        .ok_or_else(|| "No ELEVENLABS_API_KEY".to_string())?;
    if api_key.is_empty() {
...
async fn handle_media_image(
    ...
    if model == "flux-pro" || model == "flux-schnell" || model == "flux-realism" {
        ...
        // Handle rate limiting with key rotation
        if status == 429 {
            tracing::warn!("fal.ai rate limited, rotating key");
            mark_key_exhausted("fal", &fal_key);
            
            if let Some(new_key) = rotate_key("fal") {
                tracing::info!("Retrying fal.ai with rotated key");
                let retry_resp = client.post(format!("https://fal.run/{}", fal_model))
                    .header("Authorization", format!("Key {}", new_key))
                    .header("Content-Type", "application/json")
                    .json(&body)
                    .timeout(std::time::Duration::from_secs(120))
                    .send()
                    .await
                    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("fal.ai retry failed: {}", e)))?;
                
                if !retry_resp.status().is_success() {
                    let retry_status = retry_resp.status();
                    let retry_text = retry_resp.text().await.unwrap_or_default();
                    return Err((retry_status, format!("fal.ai error after retry: {}", retry_text)));
                }
                
                let retry_json: serde_json::Value = retry_resp.json().await
                    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to parse fal.ai retry response: {}", e)))?;
                
                images = retry_json.get("images")
                ...
                let text = resp.text().await.unwrap_or_default();
                return Err((status, format!("fal.ai rate limited, no alternate keys: {}", text)));
            }
        ...
        let api_key = get_api_key("openai")
            .ok_or_else(|| (StatusCode::SERVICE_UNAVAILABLE, "OPENAI_API_KEY not configured".to_string()))?;
        ...
        // Handle rate limiting with key rotation
        if status == 429 {
            tracing::warn!("OpenAI DALL-E rate limited, rotating key");
            mark_key_exhausted("openai", &api_key);
            
            if let Some(new_key) = rotate_key("openai") {
                tracing::info!("Retrying DALL-E with rotated key");
                let retry_resp = client.post("https://api.openai.com/v1/images/generations")
                    .header("Authorization", format!("Bearer {}", new_key))
                    .json(&body)
                    .timeout(std::time::Duration::from_secs(60))
                    .send()
                    .await
                    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("DALL-E retry failed: {}", e)))?;
                
                if !retry_resp.status().is_success() {
                    let retry_status = retry_resp.status();
                    let retry_text = retry_resp.text().await.unwrap_or_default();
                    return Err((retry_status, format!("DALL-E error after retry: {}", retry_text)));
                }
                
                let retry_json: serde_json::Value = retry_resp.json().await
                    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to parse DALL-E retry response: {}", e)))?;
                
                images = retry_json.pointer("/data")
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter().filter_map(|img| {
                ...
                let text = resp.text().await.unwrap_or_default();
                return Err((status, format!("DALL-E rate limited, no alternate keys: {}", text)));
            }
        } else if !status.is_success() {
            let text = resp.text().await.unwrap_or_default();
...
async fn poll_kling_video(
    ...
    let api_key = match get_api_key("kling") {
...
/// Handle music generation request via Stable Audio (fal.ai)
async fn handle_media_music(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(req): Json<MediaMusicRequest>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    // 1. Authenticate
    let user_id = auth_user_id(&state, &headers).await
        .ok_or_else(|| (StatusCode::UNAUTHORIZED, "Authentication required".to_string()))?;

    // 2. Validate and calculate credits
    let duration = req.duration.unwrap_or(10);
    if duration != 10 && duration != 30 && duration != 60 {
        return Err((StatusCode::BAD_REQUEST, "Duration must be 10, 30, or 60 seconds".to_string()));
    }
    
    let credits = match duration {
        10 => 10,
        30 => 20,
        60 => 40,
        _ => 10,
    ...
    // 5. Store job in DynamoDB
    let type_ = req.type_.as_deref().unwrap_or("music");
    #[cfg(feature = "dynamodb-backend")]
    {
        if let (Some(ref dynamo), Some(ref table)) = (state.dynamo_client.as_ref(), state.config_table.as_ref()) {
            create_music_job(
...
/// Background task to poll Stable Audio API via fal.ai for music generation completion
#[cfg(feature = "dynamodb-backend")]
async fn poll_stable_audio(
    dynamo: aws_sdk_dynamodb::Client,
    table: String,
    music_id: String,
    prompt: String,
    ...
    let api_key = match get_api_key("fal") {
        Some(k) if !k.is_empty() => k,
        _ => {
            tracing::error!("FAL_KEY not configured, cannot generate music");
            let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
            return;
        }
    };
    ...
    // First, submit the music generation request to fal.ai queue
    let body = serde_json::json!({
        "prompt": prompt,
    ...
    tracing::info!("Submitting music generation request: music_id={}, duration={}s", music_id, duration);

    let submit_resp = match client.post("https://queue.fal.run/fal-ai/stable-audio")
        ...
        .await
    {
        Ok(r) => r,
        Err(e) => {
            tracing::error!("Failed to submit music generation request: {}", e);
            let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
            return;
        }
    };
    ...
    if !submit_resp.status().is_success() {
        tracing::error!("Music generation submission failed with status: {}", submit_resp.status());
        let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
        return;
    }
    ...
    let submit_json: serde_json::Value = match submit_resp.json().await {
        Ok(j) => j,
        Err(e) => {
            tracing::error!("Failed to parse music generation submission response: {}", e);
            let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
            return;
        }
    };

    // Extract request_id from fal.ai queue response
    let request_id = match submit_json.get("request_id").and_then(|r| r.as_str()) {
        Some(id) => id.to_string(),
        None => {
            tracing::error!("No request_id in fal.ai response: {}", submit_json);
            let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
            return;
        }
    };

    tracing::info!("Music generation queued: music_id={}, request_id={}", music_id, request_id);
    let _ = update_music_job(&dynamo, &table, &music_id, "processing", None).await;

    // Poll for completion (60 attempts * 10s = 10 minutes max)
    for attempt in 1..=60 {
        tokio::time::sleep(tokio::time::Duration::from_secs(10)).await;

        let status_url = format!("https://queue.fal.run/fal-ai/stable-audio/requests/{}", request_id);
        let poll_resp = match client.get(&status_url)
            .header("Authorization", format!("Key {}", api_key))
            .timeout(std::time::Duration::from_secs(10))
            .send()
            .await
        {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!("Music poll attempt {} failed: {}", attempt, e);
                continue;
            }
        };

        if !poll_resp.status().is_success() {
            tracing::warn!("Music poll attempt {} returned {}", attempt, poll_resp.status());
            continue;
        }
        ...
        let poll_json: serde_json::Value = match poll_resp.json().await {
            Ok(j) => j,
            Err(e) => {
                tracing::warn!("Music poll parse error (attempt {}): {}", attempt, e);
                continue;
            }
        };

        let status = poll_json.get("status")
            .and_then(|s| s.as_str())
            .unwrap_or("unknown");

        tracing::debug!("Music poll attempt {}: status={}", attempt, status);

        // Check for completion
        if status == "COMPLETED" {
            // Extract audio URL
            let audio_url = poll_json.pointer("/output/audio_file/url")
                .or_else(|| poll_json.get("audio_file").and_then(|a| a.get("url")))
                .and_then(|u| u.as_str());

            if let Some(url) = audio_url {
                tracing::info!("Music generation completed: music_id={}, url={}", music_id, url);
                let _ = update_music_job(&dynamo, &table, &music_id, "completed", Some(url)).await;
                return;
            } else {
                tracing::error!("Music completed but no URL found: {}", poll_json);
                let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
                return;
            }
        } else if status == "FAILED" {
            tracing::error!("Music generation failed: {}", poll_json);
            let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
            return;
        }
    ...
    tracing::error!("Music generation timed out after 60 polling attempts: music_id={}", music_id);
    let _ = update_music_job(&dynamo, &table, &music_id, "failed", None).await;
}

/// Get music generation job status
async fn handle_media_music_status(
    State(state): State<Arc<AppState>>,
    _headers: axum::http::HeaderMap,
...
/// POST /api/v1/feedback — Collect user feedback (thumbs up/down)
